...
import { mapGetXApiResponseToPosts } from "./getxapi-mapper";
...
export class GetXapiXProvider implements ProviderAdapter {
  readonly id = "getxapi";
  readonly name = "GetXAPI X Provider Sandbox";
  ...
  constructor(options: GetXapiXProviderOptions) {
    ...
    this.baseUrl = options.baseUrl ?? "https://api.getxapi.local/x";
  ...
  private async fetchFromSandbox(value: string, sourceType: SourceType, options: ProviderFetchOptions): Promise<ProviderFetchResponse> {
    if (!this.httpClient) {
      throw providerError({
        ...
        providerId: this.id,
        message: "GetXAPI sandbox provider requires an injected ProviderHttpClient."
      });
    }
    ...
    const mapped = mapGetXApiResponseToPosts(response.data, this.id);
